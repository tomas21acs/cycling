...
def solve_speed_for_power(grade_percent: float, target_power: float, mass_total: float) -> Tuple[float, bool]:
    """Find speed where total power equals target_power using bisection.

    Returns a tuple (speed_mps, limited) where limited indicates solver hitting bounds.
    """
    ...
    if f_low > 0 and grade_percent >= 0:
        return low, True
    if f_high < 0:
        return high, True
...
def coasting_speed(grade_percent: float, mass_total: float) -> float:
    ...
    speed, _ = solve_speed_for_power(grade_percent, 0.0, mass_total)
    return speed
...
def calculate_segments(
    ...
    if mode == "speed":
        ...
        percent = RPE_TO_PERCENT.get(target_rpe, 1.0)
        target_power = ftp * percent
    else:
    ...
    for idx, seg in enumerate(segments, start=1):
        ...
        if mode == "speed":
            ...
            speed_mps_eff, limited = solve_speed_for_power(grade, target_power, mass_total)
            power = max(target_power, 0.0)
            time_seg = seg.delta_distance / speed_mps_eff
            speed_kmh = speed_mps_eff * 3.6
            power_display = f"{power:.0f}" + ("*" if limited else "")
    ...
    return CalculationResult(segments=segment_results, summary=summary, chart=chart)
